...
void loop() {
 ...
 if(replyIf1 == "49") { // When laptop sends "1"  //ASCII value for 1.
  ...
  sendSerialAsTwoBytes(bmsTemp2); // Takes a float up to 6548. Sends it to 1 decimal place.
...
void sendSerialAsTwoBytes(float value) {
  unsigned int decimalValue = round(value * 10); // Send with one decimal place. Currently only sending whole integers anyway.
